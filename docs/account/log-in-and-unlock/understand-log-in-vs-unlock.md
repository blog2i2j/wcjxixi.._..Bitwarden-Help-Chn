...
* 登录始终需要使用您的主密码、[已批准的设备](more-log-in-options/log-in-with-device.md)或[已创建的通行密钥](more-log-in-options/log-in-with-passkeys.md)，以获取解密密码库数据所需的[账户加密密钥](../../security/encryption/encryption-key-rotation.md)。在此阶段也需要使用[任何已启用的两步登录方式](../two-step-login/setup-two-step-login/two-step-login-methods.md)。
