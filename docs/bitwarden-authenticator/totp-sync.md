...
将 Authenticator 与 Password Manager 连接以同步您的验证码。激活后，Bitwarden App 将与您添加或编辑的任何代码保持同步（本地代码除外）。这可以帮助您从任一 App 快速查找和使用 TOTP：
...
如果您有多个 Password Manager 账户，您可以选择与 Authenticator 同步的账户。相反，存储在 Authenticator 中的本地代码可以保持独立，或者将它们移动到 Password Manager 以便从两个 App 进行访问。

{% hint style="info" %}
如果您希望仅将验证码保存在一个 Bitwarden App 中，请不要同步您的账户。相反，有两种选择：

* **仅从 Bitwarden 密码库访问代码**：使用 Password Manager 集成的身份验证器保存 TOTP，并且不要打开 Bitwarden 账户的**允许验证器同步** App 设置。这意味着该特定 Bitwarden 账户中保存的任何代码都不会在 Authenticator 中可见。
...
已作为登录项目保存在 Bitwarden 密码库中的代码可以[与 Authenticator 同步](totp-sync.md#sync-codes-saved-in-your-bitwarden-vault)。如果您有本地代码，即仅保存在您的设备上并通过 Authenticator 访问的 TOTP，请首先[将代码复制到您的密码库](totp-sync.md#move-and-sync-local-codes-to-your-bitwarden-vault)。代码位于您的密码库中后，它们就会像其他 TOTP 一样同步。
...
1. 确保您的设备上安装了 Bitwarden Authenticator 和 Bitwarden Password Manager。
2. 在 Password Manager 中，登录要与 Authenticator 同步的账户。
3. 点击 **⚙️设置**图标。
4. 点击**账户安全**。
5. 启用**允许验证器同步**。
6. （可选）要同步其他 Password Manager 账户，请为每个账户重复步骤 2-5。您需要为每个账户单独切换设置，这样您就可以准确选择与 Authenticator 同步的账户。
7. Authenticator 将代码分为两组：本地代码和来自 Password Manager 的同步代码。验证您的 Password Manager TOTP 代码是否显示在 Authenticator 中您的账户电子邮件标题下。
...
当您关闭**允许验证器同步**或完全注销 Bitwarden Password Manager 时，存储在 App Group 中的加密数据（验证器密钥、显示名称和用户名）将被删除。如果所有关联的 Bitwarden 账户停用**允许验证器同步**或注销，则**全局对称密钥**也会被删除。
