...
Docker 配置完成后，我们可以继续 Linux 自托管安装过程。对于每一种部署指南（[Linux 标准部署](linux-standard-deployment.md)、[Linux 手动部署](linux-standard-deployment-1.md)和 [Linux 离线部署](linux-offline-deployment.md)），用户将在**创建 Bitwarden 本地用户和目录**步骤**之后**，开始完成自托管安装。
