...
{% hint style="info" %}
正向代理的配置仅应由高级用户完成。本指南不涉及正向代理本身的设置。
{% endhint %}

本文演示了将 Bitwarden 自托管实例的流量路由到正向代理所需的必要配置。目前，仅 Linux Docker Compose 环境支持此代理环境。

{% hint style="info" %}
配置正向代理前，请先完成 Bitwarden 自托管实例的部署步骤（包括[创建 Bitwarden 本地用户和目录](linux-standard-deployment.md#create-bitwarden-local-user-and-directory)）。
{% endhint %}
...
通过**创建 Bitwarden 本地用户和目录**步骤配置您的自托管环境后，您可以返回到本指南并继续进行正向代理配置。
...
现在您已将 Docker 配置为通过正向代理发送流量，正向代理设置将需要额外的客户端配置：
...
Docker 配置完成后，我们可以继续 Linux 自托管安装过程。对于每一种部署指南（[Linux 标准部署](linux-standard-deployment.md)、[Linux 手动部署](linux-standard-deployment-1.md)和 [Linux 离线部署](linux-offline-deployment.md)），用户需在完成**创建 Bitwarden 本地用户和目录**步骤后，继续执行后续操作以完成自托管安装。
