...
客户端应用程序和自托管服务器会在服务器发布后几天内发布，以确保稳定性。通过应用程序商店分发的 App 需经过分发商的额外审批流程。因此，**客户端应用程序和自托管服务器通常会在本页面发布公告后，陆续获得新功能**。
...
* **适用于 Linux 的 Arm64 构建**：Snapcraft 和 Flathub 上的 Linux 用户现在可以使用 Arm64 桌面版构建。参阅[下载页面](https://bitwarden.com/download/#downloads-desktop-applications)获取更多信息。

### 管理控制台 <a href="#admin-console" id="admin-console"></a>

* **★自动确认策略**：一个新的策略，允许[自动确认新成员加入组织](admin-console/manage-members/automatic-confirmation.md)。使用此策略需要了解其工作原理，并且必须联系 Bitwarden 支持人员才能在您的组织中激活它。
* **★策略名称更新**：「强制组织数据所有权」策略已重命名为「[集中组织所有权](admin-console/oversight-visibility/enterprise-policies.md#centralize-organization-ownership)」。
* **★从「我的密码库」转移项目到「我的项目」**：使用集中组织所有权策略的组织现在可以[选择提示用户](admin-console/oversight-visibility/enterprise-policies.md#prompt-users-to-move-items-to-the-organization)使用浏览器扩展[将项目从「我的密码库」转移项目到「我的项目」](password-manager/organization-members/transfer-ownership.md)。
