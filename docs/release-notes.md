...
* **Android 预览图片附件**：现在，您可以在 Password Manager Android App 中直接预览图片附件，而无需将文件下载到设备。未来版本中，iOS 也将提供预览图片附件功能。
...
* **更新了企业策略页面**：在策略页面中，[企业策略](admin-console/oversight-visibility/enterprise-policies.md)现已分为三个类别：数据控制、身份验证和密码库管理。您还可以一目了然地查看哪些策略已启用或已禁用。
* **Access Intelligence 仪表板更新**：在 Access Intelligence 活动视图中，通过新的图示，可视化[存在风险的应用程序、密码和成员随时间推移的变化趋势](admin-console/oversight-visibility/access-intelligence.md#activity)，并与应用程序、密码和成员数量的绝对变化进行比较。
