...
Bitwarden 组织以可扩展且安全的方式管理成员和数据。单独管理成员和数据对于大型企业来说效率低下，并且容易出错。为了解决这个问题，组织提供了集合和[群组](onboarding-and-succession.md#collections)功能。
...
Directory Connector、SSO 登录、企业策略和您的密码库可以独立或协调工作，以优化您的入职、继任和组织管理体验。下表详细说明了将这些部分串在一起形成一个平滑过程的方式：
